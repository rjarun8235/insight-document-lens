  ...
  const processDocuments = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        setState(prev => ({
          ...prev,
          progress: Math.round((i / (totalFiles * 2)) * 100),
          processingStatus: `Parsing document ${i + 1} of ${totalFiles}: ${file.name}`
        }));
        ...
        try {
          ...
          if (workerSupportedRef.current && ['csv', 'excel', 'pdf'].includes(file.type)) {
            content = await new Promise((resolve, reject) => {
              workerService.parseDocument(
                ...
                (progress, metadata) => {
                  ...
                  setState(prev => {
                    ...
                    return {
                      ...prev,
                      files: updatedFiles,
                      processingStatus: metadata?.currentPage 
                        ? `Parsing ${file.name}: Page ${metadata.currentPage} of ${metadata.numPages || '?'}`
                        : `Parsing ${file.name}: ${progress}%`
                    };
                  });
                },
                (preview) => {
                ...
                (error, needsFallback) => {
                  if (needsFallback && file.type === 'pdf') {
                    // Try fallback for PDFs
                    console.log(`Worker failed for PDF ${file.name}, using fallback`);
                    parseDocument(file)
                      .then(resolve)
          ...
          if (file.type === 'pdf') {
            setState(prev => ({
              ...prev,
              processingStatus: `Using vision fallback for ${file.name}`
            }));
